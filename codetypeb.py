...
if hist_criteria:
  ...
  if st.sidebar.button("Historical Data", key="historical_data"):
    ...
    data_df02 = data_df01.style.apply(highlight_second_highest, subset=['Open','High', 'Low','Close']).format(precision=2).map(color_all, subset=['Volume']).map(color_two, subset=['Time'])
