...
if hist_criteria:
  ...
  if st.sidebar.button("Historical Data", key="historical_data"):
    ...
    data_df02 = data_df01.style.apply(highlight_second_highest, subset=['Open','High', 'Low']).format(precision=2).map(color_all, subset=['Volume']).map(color_two, subset=['Close'])
...
strikes_range= st.number_input("strike range from ATM", value= 200, key='strike_r')
...
PE_details1 = PE_details.rename(columns={
            'Open':'PE_Open',
            'High':'PE_High',
            'Low':'PE_Low',
            'Close':'PE_Close',
...
show_optin = option_merged_final[['Time_x','CE_OI', 'CE_volume', 'CE_Close', 'CE.strike','PE_Close','PE_volume', 'CE.expiry']]
